...
self.addEventListener("fetch", (event) => {
  ...
  if (
    ...
    if (
      ...
      event.respondWith(networkFirst(event.request, assetsCacheName, true));
      return;
    }

    event.respondWith(networkOnly(event.request, true));
    return;
  }
...
async function cacheOnly(req, cacheName, isRequestingHTML = false) {
  ...
  const response = await fetchReq(req, cache);
...
async function staleWhileRevalidate(req, cacheName) {
  const cache = await caches.open(cacheName);
  const cachedResponse = await cache.match(req);
  const fetchRes = await fetchReq(req, cache, true);
...
async function networkOnly(req, isRequestingHTML = false) {
  const response = fetchReq(req);
  if (response) return response;
